...
#### What is the most popular name for pets ?
```{r}
...
catbyname
```
TOP 10 Most Popular Cats' Name:Kittens,Kitty,Baby,Mimi,Kitten,Ginger,Tiger,Kiki,Snowy,Blackie
...
dogbyname
```
TOP 10 Most Popular Dogs' Name:Lucky,Puppy,Brownie,Max,Baby,Blackie,Coco,Bobby,Puppies, Angel

#### What are the names for pets that got adopted more faster than others?
...
Among the top 10 popular names for cats, Blackie, Mimi, Tiger, Kitty and Kitten are top 5 names, such that cats with those names can adopt faster than others.
...
Among the top 10 popular names for dogs, Lucky,puppies, Blackie, Bobby and Maxare top 5 names, such that dogs with those names can adopt faster than others.
...
We checked all the affecting variables and substituted the breed factor with mixed breed factor. After building two SVM Models (one with vanilladot, one with rbfdot), we discovered that the rbfdot possessed higher accuracy. Hence we chose rbfdot SVM model for our prediction which gives an accuracy rate of `r svm_acc`.
...
train(AdoptionSpeed ~ ., data = test, method = "treebag",
      trControl = ctrl)
```
The mode's accuracy is 0.6205, which seems to have fit the training data well. And the kappa statistic of 0.21 for this model suggests that the bagged tree model performs on par with our C5.0 decision tree.
